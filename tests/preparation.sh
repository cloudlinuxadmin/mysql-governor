...
mysql --silent --host=localhost --user=root --execute "grant all privileges on *.* to mysqldd@localhost identified by 'WRT56GL' with grant option;" > /dev/null 2>&1
sleep 5
...
mysql --silent --host=localhost --user=root --execute "grant all privileges on *.* to mytestsqld@localhost identified by 'WRT56GL' with grant option;" >/dev/null 2>&1
sleep 5
...
mysql --silent --host=localhost --user=root --execute "grant all privileges on *.* to mysqlslaptest@localhost identified by 'WRT56GL' with grant option;"
sleep 5
