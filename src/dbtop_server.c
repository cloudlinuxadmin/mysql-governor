...
void *run_server(void *data) {
    ...
    if ((s = socket(AF_UNIX, SOCK_STREAM, 0)) < 0) {
        WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "Can't create socket(DBTOP)", data_cfg.log_mode);
        close_log();
        close_restrict_log();
        exit(EXIT_FAILURE);
    }
    ...
    if (bind(s, (struct sockaddr *) &saun, len) < 0) {
        WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "Can't server bind(DBTOP)", data_cfg.log_mode);
        close_log();
        close_restrict_log();
        close(s);
        exit(EXIT_FAILURE);
    ...
    if (listen(s, 3) < 0) {
        WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "Can't server listen(DBTOP)", data_cfg.log_mode);
        close_log();
        close_restrict_log();
        close(s);
        exit(EXIT_FAILURE);
    ...
    while (1) {
        if ((ns = accept(s, (struct sockaddr *) &fsaun, &fromlen)) < 0) {
            if (errno == EINTR) {
                ...
                WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "Can't server accept(DBTOP)", data_cfg.log_mode);
                close_log();
                close_restrict_log();
                exit(EXIT_FAILURE);
            }
        ...
        if (ctt == DBTOP) {
            ret = pthread_create(&thread, NULL, run_writer, (void *) accept_socket);
            pthread_detach(thread);
        } else if (ctt == DBCTL) {
            ret = pthread_create(&thread, NULL, run_dbctl_command, (void *) accept_socket);
            pthread_detach(thread);
        } else if (ctt == DBTOPCL) {
            ret = pthread_create(&thread, NULL, run_dbtop_command, (void *) accept_socket);
            pthread_detach(thread);
        } else {
            WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "incorrect connection(DBTOP)", data_cfg.log_mode);
            close(ns);
        }
    }
}
    ...
    while (1) {
        if (flag_need_to_renew_dbmap) {
            ...
            if ((last_renew + DBMAPHOOK_ANTIDDOS) < current_check) {
                ...
                if (renew_pid < 0) {
                    WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "(%d)Fork error (renew dbmap). Path %s", data_cfg.log_mode, errno, "dbupdate");
                } else {
                    if (!renew_pid) {
                        execl("/usr/share/lve/dbgovernor/mysqlgovernor.py", "/usr/share/lve/dbgovernor/mysqlgovernor.py", "--dbupdate", NULL);
                        WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "(%d)Exec error (renew dbmap). Path %s", data_cfg.log_mode, errno, "dbupdate");
                        exit(0);
                    }
                }
            }
