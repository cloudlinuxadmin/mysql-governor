...
char *get_mb_str(char *s, char *buf, int flag) {
...
char *GetUserName(GHashTable *attr) {
    return (char *) g_hash_table_lookup(attr, "name");
}

char *GetUserMysqlName(GHashTable *attr) {
    return (char *) g_hash_table_lookup(attr, "mysql_name");
}
...
void ReadCfg(char *file_name, char *tag) {
    ...
    const char *ptr;
    xml_data *xml = parseConfigData(file_name, xml_parse_error, MAX_XML_PATH - 1);
    ...
    for (tmp_xml = getNextChild(xml->root, tag, NULL); tmp_xml; tmp_xml = getNextChild(xml->root, tag, tmp_xml)) {
        ...
        for (tmp_xml_attr = getNextAttr(tmp_xml, NULL); tmp_xml_attr; tmp_xml_attr = getNextAttr(tmp_xml, tmp_xml_attr))
            g_hash_table_insert(found_tag->attr, getAttributeName(tmp_xml_attr), getAttributeValue(tmp_xml_attr));
        ...
        for (tmp_xml_limit = getNextChild(tmp_xml, "limit", NULL); tmp_xml_limit; tmp_xml_limit = getNextChild(tmp_xml, "limit", tmp_xml_limit)) {
            ...
            if (attr_value) {
                strncpy(dbctl_l_attr->l_name, attr_value, sizeof (dbctl_l_attr->l_name) - 1);
                releaseElemValue(attr_value);
            }

            attr_value = getElemAttr(tmp_xml_limit, "current");
            if (attr_value) {
                strncpy(dbctl_l_attr->l_current, attr_value, sizeof (dbctl_l_attr->l_current) - 1);
                releaseElemValue(attr_value);
            }

            attr_value = getElemAttr(tmp_xml_limit, "short");
            if (attr_value) {
                strncpy(dbctl_l_attr->l_short, attr_value, sizeof (dbctl_l_attr->l_short) - 1);
                releaseElemValue(attr_value);
            }

            attr_value = getElemAttr(tmp_xml_limit, "mid");
            if (attr_value) {
                strncpy(dbctl_l_attr->l_mid, attr_value, sizeof (dbctl_l_attr->l_mid) - 1);
                releaseElemValue(attr_value);
            }

            attr_value = getElemAttr(tmp_xml_limit, "long");
            if (attr_value) {
                strncpy(dbctl_l_attr->l_long, attr_value, sizeof (dbctl_l_attr->l_long) - 1);
                releaseElemValue(attr_value);
            }
...
char *GetDefaultForUsers(GPtrArray * tags, DbCtlLimitAttr * cpu_def, DbCtlLimitAttr * read_def, DbCtlLimitAttr * write_def, int flag) {
    ...
    for (; i < (cpu_def ? tags->len : 1); i++) {
        char *buffer_name = (char *) alloca(16 * sizeof (char)), *buffer_data = (char *) alloca(90 * sizeof (char));
        ...
        if (strcmp(mode, "ignore") != 0) {
            ...
            int cpu_short = atoi(GetLimitAttr(found_tag_->limit_attr, "cpu", "short"));
            int cpu_mid = atoi(GetLimitAttr(found_tag_->limit_attr, "cpu", "mid"));
            int cpu_long = atoi(GetLimitAttr(found_tag_->limit_attr, "cpu", "long"));

            long long read_curr_real_size = atoll(GetLimitAttr(found_tag_->limit_attr, "read", "current"));
            long long read_short_real_size = atoll(GetLimitAttr(found_tag_->limit_attr, "read", "short"));
            long long read_mid_real_size = atoll(GetLimitAttr(found_tag_->limit_attr, "read", "mid"));
            long long read_long_real_size = atoll(GetLimitAttr(found_tag_->limit_attr, "read", "long"));

            long long write_curr_real_size = atoll(GetLimitAttr(found_tag_->limit_attr, "write", "current"));
            long long write_short_real_size = atoll(GetLimitAttr(found_tag_->limit_attr, "write", "short"));
            long long write_mid_real_size = atoll(GetLimitAttr(found_tag_->limit_attr, "write", "mid"));
            long long write_long_real_size = atoll(GetLimitAttr(found_tag_->limit_attr, "write", "long"));

            long long read_curr = atoll(get_mb_str(GetLimitAttr(found_tag_->limit_attr, "read", "current"), mb_buffer, flag));
            long long read_short = atoll(get_mb_str(GetLimitAttr(found_tag_->limit_attr, "read", "short"), mb_buffer, flag));
            long long read_mid = atoll(get_mb_str(GetLimitAttr(found_tag_->limit_attr, "read", "mid"), mb_buffer, flag));
            long long read_long = atoll(get_mb_str(GetLimitAttr(found_tag_->limit_attr, "read", "long"), mb_buffer, flag));

            long long write_curr = atoll(get_mb_str(GetLimitAttr(found_tag_->limit_attr, "write", "current"), mb_buffer, flag));
            long long write_short = atoll(get_mb_str(GetLimitAttr(found_tag_->limit_attr, "write", "short"), mb_buffer, flag));
            long long write_mid = atoll(get_mb_str(GetLimitAttr(found_tag_->limit_attr, "write", "mid"), mb_buffer, flag));
            long long write_long = atoll(get_mb_str(GetLimitAttr(found_tag_->limit_attr, "write", "long"), mb_buffer, flag));
            ...
            if (flag) {
                ...
                snprintf(buffer_cpu, 24, "%d/%d/%d/%d", cpu_curr, cpu_short, cpu_mid, cpu_long);
                gchar * tmp_param[4] = {0};
                tmp_param[0] = g_strdup_printf("%i", read_curr);
                tmp_param[1] = g_strdup_printf("%i", read_short);
                tmp_param[2] = g_strdup_printf("%i", read_mid);
...
void *SearchTagByName(xml_data *cfg, char *name_tag, char *name) {
    ...
    for (tmp_xml = getNextChild(cfg->root, name_tag, NULL); tmp_xml; tmp_xml = getNextChild(cfg->root, name_tag, tmp_xml)) {
        if (!name)
            return tmp_xml;
