...
int valid_comm(int argc, char **argv) {
    ...
    name_comm level_100[] = {"list", "list-restricted", "unrestrict-all", "list-restricted-shm", "dbupdate"};

    char _tmp_arg[11];
    _tmp_arg[0] = '\0';
    strlcpy(_tmp_arg, argv[1], sizeof (_tmp_arg));

    if (strcmp("--help", argv[1]) == 0 || strcmp("--version", argv[1]) == 0 || strcmp("--lve-mode", _tmp_arg) == 0)
        return 1;
    ...
    if (strcmp("set", argv[1]) == 0) {
        if (argc > 2) {
            ...
            if (strcmp("default", _argv) == 0)
                setDefault((char *) GetVal('c', list), (char *) GetVal('r', list),
                    (char *) GetVal('w', list), (char *) GetVal('s', list));
            else
                setUser(_argv, (char *) GetVal('c', list), (char *) GetVal('r', list),
                    (char *) GetVal('w', list), (char *) GetVal('s', list));
        } else
            return 1;
