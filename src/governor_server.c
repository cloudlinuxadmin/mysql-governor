...
void create_socket() {
    ...
    if ((global_socket = socket(AF_UNIX, SOCK_STREAM, 0)) < 0) {
        WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "Can't create socket", data_cfg.log_mode);
        close_log();
        close_restrict_log();
        exit(EXIT_FAILURE);
    }
    ...
    if (setsockopt(global_socket, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof opt) < 0) {
        WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "Can't change socket options", data_cfg.log_mode);
        close_log();
        close_restrict_log();
        exit(EXIT_FAILURE);
    }
    ...
    if (bind(global_socket, (struct sockaddr *) &saun, len) < 0) {
        WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "Can't bind to socket address %s", data_cfg.log_mode, SOCK_ADDRESS);
        close_log();
        close_restrict_log();
        exit(EXIT_FAILURE);
    }
    ...
    if (listen(global_socket, 32) < 0) {
        WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "Can't listen on socket", data_cfg.log_mode);
        close_log();
        close_restrict_log();
        exit(EXIT_FAILURE);
    }
...
void *get_data_from_client(void *data) {
    ...
    for (;;) {
        ...
        if (ret == -1) {
            ...
            WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "Error on polling socket. Recreating socket", data_cfg.log_mode);
        ...
        for (i = 0; (i < nfds) && (ret); i++) {
            ...
            if (((fds + i)->fd == get_soket()) && ((fds + i)->revents & POLLIN)) {
                /*
                 * Accept connection from socket:
                 * accepted connection will be on socket (fds+nfds)->fd.
                 */
                struct pollfd *fds_tmp = NULL;
                fds_tmp = (struct pollfd *) realloc(fds, (nfds + 1) * sizeof (struct pollfd));
                if (fds_tmp)
                    fds = fds_tmp;

                (fds + nfds)->fd = accept(global_socket, (struct sockaddr *) &fsaun, &fromlen);
#ifdef TEST
                //printf("Get accept descriptor %d\n", i, (fds + nfds)->fd);
#endif
                if ((fds + nfds)->fd == -1) {
                    WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "Error on polling socket. Accepting error", data_cfg.log_mode);
...
#endif
                remove_tid_data_by_fd((fds + i)->fd);
                WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "Error on polling socket. Error %d", data_cfg.log_mode, errno);
                cleanup(0, (fds + i)->fd, 2);
                nfds--;
                memcpy(fds + i, fds + i + 1, (nfds - i) * sizeof (struct pollfd));
                struct pollfd *fds_tmp = NULL;
            ...
            if ((fds + i)->revents & POLLIN) {
                ...
                retval_fcntl = fcntl((fds + i)->fd, F_SETFL, fcntl((fds + i)->fd, F_GETFL) | O_NONBLOCK);
                //retval = recv((fds + i)->fd, &message, sizeof(message), 0);
                retval = recv_wrapper((fds + i)->fd, &message, sizeof (message), 0);
                if (retval <= 0 || retval_fcntl < 0) {
                    ...
                    if (retval == 0) {
                        ...
                        WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "Error on polling socket. Read %d", data_cfg.log_mode, errno);
                        cleanup(0, (fds + i)->fd, 1);
                    }
                    nfds--;
                    memcpy(fds + i, fds + i + 1, (nfds - i) * sizeof (struct pollfd));
                    ...
                    if (data_cfg.restrict_format >= 4) {
                        WRITE_LOG(NULL, 1, buffer, _DBGOVERNOR_BUFFER_2048,
                                ...
                                data_cfg.log_mode, (fds + i)->fd, message.type, message.tid,
                                message.username, message.cpu, message.write, message.read);
                    }
...
void chek_user_perf(gpointer key, tid_table * item, gpointer user_data) {
    ...
    double old_tm = item->update_time + (double) item->naoseconds / (double) SEC2NANO;
    struct timespec cur_tm;
    ...
    if (data_cfg.restrict_format >= 4) {
        WRITE_LOG(NULL, 1, buffer, _DBGOVERNOR_BUFFER_2048,
                ...
                data_cfg.log_mode, kkey, item->username, item->cpu, item->write, item->read, old_tm);
    }
    if ((new_tm - old_tm) > 1.0) {
        dbgov_proc_time item1;
        dbgov_iostat item2;

        if ((get_proc_time(&item1, item->pid, kkey) == -2) || (get_io_stat(&item2, item->pid, kkey) == -2)) {
        ...
        if (data_cfg.restrict_format >= 4) {
            WRITE_LOG(NULL, 1, buffer, _DBGOVERNOR_BUFFER_2048,
                    ...
                    data_cfg.log_mode, kkey, item->username, item->cpu, item->write, item->read,
                    old_tm, item1.utime + item1.stime, item2.write_bytes, item2.read_bytes, new_tm);
        }
#ifdef TEST
        //      struct timespec cur_tm11;
        //      clock_gettime(CLOCK_REALTIME, &cur_tm11);
        //double new_tm = cur_tm11.tv_sec + (double) cur_tm11.tv_nsec / (double) SEC2NANO;
        //printf("Get cpu1 %ld, tid %d, tm %f\n", item1.utime + item1.stime, kkey, new_tm);
#endif

        clac_stats_difference_inner_add_to_counters(item1.utime + item1.stime, item2.read_bytes, item2.write_bytes, item);
...
void monitor_data_from_client(void *data) {
    ...
    get_config_data(&data_cfg);
    if (data_cfg.restrict_format >= 4)
        WRITE_LOG(NULL, 1, buffer, _DBGOVERNOR_BUFFER_2048, "TID Table size %ld", data_cfg.log_mode, get_tid_size());
