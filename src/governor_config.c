...
static int _set_val(T_LONG * limit, const char *ptr, const char *name, long *value) {
...
static void set_stats_limit(void *inner_xml, stats_limit_cfg * st, xml_data *xml) {
...
void config_add_work_user(const char *user_name) {
    if (user_name && (user_name[0] != 0)) {
        stats_limit_cfg *ptr = g_hash_table_lookup(cfg->account_limits, user_name);
        if (!ptr) {
            ...
            g_hash_table_replace(cfg->account_limits, (gpointer) strdup(user_name), l);
        }
    }
...
struct governor_config *config_init(const char *path) {
    ...
    const char *ptr;
    xml_data *xml = parseConfigData(path, (char *) xml_parse_error, MAX_XML_PATH - 1);
    if (xml == NULL) {
    ...
    cfg->account_limits = g_hash_table_new_full(g_str_hash, g_str_equal, (GDestroyNotify) free, (GDestroyNotify) free);
    ...
    if (ptr) {
        ...
        if (cfg->exec_script) {
            if (!cfg->exec_script[0]) {
                ...
                if (status_script) {
                    fprintf(stderr, "Wrong script name - %s. Work without script\n", cfg->exec_script);
                    free(cfg->exec_script);
                    cfg->exec_script = NULL;
                } else {
    ...
    for (tmp_xml = getNextChild(xml->root, "user", NULL); tmp_xml; tmp_xml = getNextChild(tmp_xml, "user", tmp_xml)) {
        ...
        if ((account == NULL) && (mysql_name == NULL)) {
            releaseConfigData(xml);
            fprintf(stderr, "Error: both 'name' and 'mysql_name' attributes are absent\n");
            exit(-1);
        }
        ...
        if ((account != NULL) && (mysql_name != NULL)) {
            ...
            releaseConfigData(xml);
            fprintf(stderr, "Error: both 'name' and 'mysql_name' attributes are present\n");
            exit(-1);
        }
        ...
        g_hash_table_replace(cfg->account_limits, (gpointer) strdup((account == NULL) ? mysql_name : account), l);
...
int save_duplicate_config(xml_data *xml) {
    void *tmp_xml = FindElementWithName(xml, NULL, "connector");
...
static void print_account_configs(gpointer key, gpointer value, gpointer user_data) {
    ...
    printf("%s_MODE %s\n", (char *) key, mode_type_enum_to_str(ptr->mode, buffer, 511));
}
...
void print_config_full(void) {
    ...
    printf("LVE %s, USE_LVE %d, ALL_LVE %d, SEP_LVE %d\n", buffer, cfg->use_lve, cfg->all_lve, cfg->separate_lve);
    printf("LOG %s, MODE %s\n", cfg->log ? cfg->log : "NULL", mode_type_enum_to_str(cfg->log_mode, buffer, 511));
