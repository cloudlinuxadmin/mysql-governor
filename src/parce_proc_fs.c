...
static inline char *next_token(const char *p) {
    while (isspace(*p))
        p++;
    return (char *) p;
}
...
static int set_linux_version() {
    ...
    if (LINUX_VERSION_CODE(x, y, z) >= LINUX_VERSION_CODE(2, 6, 0) && !has_sysfs())
        return -1;
...
static int try_file_to_buffer(char *buffer, const char *format, ...) {
...
int get_io_stat(dbgov_iostat * info, pid_t pid, pid_t tid) {
    ...
    if (try_file_to_buffer(buffer, "/proc/%d/task/%d/io", pid, tid) != TRY_FILE_TO_BUFFER_OK_IOSTAT)
        return -2;
    ...
    info->cancelled_write_bytes = get_scaled_iostat(buffer, "cancelled_write_bytes:");
    info->write_bytes = (info->cancelled_write_bytes <= info->write_bytes) ? (info->write_bytes - info->cancelled_write_bytes) : 0;
...
void get_cpu(dbgov_cpu * buf) {
    ...
    if (try_file_to_buffer(buffer, FILENAME) != TRY_FILE_TO_BUFFER_OK_IOSTAT)
        return;
...
static inline char *proc_stat_after_cmd(char *p) {
    p = strrchr(p, ')');
...
static inline char *skip_multiple_token(const char *p, size_t count) {
    while (count--)
        p = skip_token(p);
...
int get_proc_time(dbgov_proc_time * buf, pid_t pid, pid_t tid) {
    ...
    if (try_file_to_buffer(buffer, "/proc/%d/task/%d/stat", pid, tid) != TRY_FILE_TO_BUFFER_OK_IOSTAT)
        return -2;
...
void get_mem(dbgov_mem * buf) {
    ...
    if (try_file_to_buffer(buffer, FILENAMEMEM) != TRY_FILE_TO_BUFFER_OK_IOSTAT)
        return;
...
void get_proc_mem(dbgov_proc_mem * buf, pid_t pid, pid_t tid) {
    ...
    if (!(p = proc_stat_after_cmd(buffer)))
        return;

    p = skip_multiple_token(p, 20);

    buf->vsize = strtoull(p, &p, 0);
    buf->rss = strtoull(p, &p, 0) * pagesize;
    buf->rss_rlim = strtoull(p, &p, 0);

    if (try_file_to_buffer(buffer, "/proc/%d/task/%d/statm", pid, tid) != TRY_FILE_TO_BUFFER_OK_IOSTAT)
        return;

    buf->size = strtoull(buffer, &p, 0) * pagesize;
    buf->resident = strtoull(p, &p, 0) * pagesize;
    buf->share = strtoull(p, &p, 0) * pagesize;
}
