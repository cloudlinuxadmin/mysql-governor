...
static int connection_with_timeout_select(int sk, struct sockaddr_un *sa, socklen_t len, int timeout) {
...
void *governor_load_lve_library() {
    ...
    if (lve_library_handle) {
        while (1) {
            ...
            lve_enter_pid = (int (*)(void *, uint32_t, pid_t)) dlsym(lve_library_handle, "lve_enter_pid");
            if ((error_dl = dlerror()) != NULL) {
                init_lve = NULL;
                destroy_lve = NULL;
                lve_enter_flags = NULL;
