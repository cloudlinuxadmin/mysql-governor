...
void get_mb(char **s) {
    long long mb = (long long) (atoll(*s)) * (long long) (1024 * 1024);
    if (mb < 0) mb = -1;
    sprintf(*s, "%lld", mb);
}

void get_kb(char **s) {
    long long kb = (long long) (atoll(*s)) * (long long) (1024);
    if (kb < 0) kb = -1;
    sprintf(*s, "%lld", kb);
}
...
void *addLimit(xml_data *xml, void *child, char *n, char *s) {
    return setLimitAttr(setNodeWithAttr(xml, child, "limit", NULL, "name", n), s);
}

int setDefault(char *cpu, char *read, char *write, char *slow) {
    char xml_parse_error[MAX_XML_PATH] = {0};
    xml_data *cfg = parseConfigData((char *) CONFIG_PATH, xml_parse_error, MAX_XML_PATH - 1);
    if (cfg == NULL) {
