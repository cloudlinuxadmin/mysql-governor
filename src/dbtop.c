...
char *getRestrictInfo(Account * ac, char *buffer) {
    char ch;
...
void printHeader() {
    ...
    clear();
    getmaxyx(stdscr, screen_height, screen_weight);
    printString("h,? - help; z - toggle color-mode; q,F10,Ctrl-c - quit", A_NORMAL, getW(), NEWLINE);

    if (is_colorize)
        attron(COLOR_PAIR(1));

    sprintf(header_buf, " +User%c          .+cpu(%%)  %c         . +read(B/s)%c        . +write(B/s)   %c . CAUSE    ",
            sort_type == 3 ? '*' : ' ', !sort_type ? '*' : ' ', sort_type == 1 ? '*' : ' ', sort_type == 2 ? '*' : ' ');
