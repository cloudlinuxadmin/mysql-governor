		   ...
		   char *argv[], int save_global)
{
  ...
  {"user", optional_argument, NULL, 'g'}, {"password",
					   optional_argument, NULL, 'j'},
    {"pass",
     optional_argument, NULL, 'i'}, {"socket",
				     optional_argument, NULL, 'S'}, {0, 0, 0,
								     0}
  };
  ...
  if (*internal_db != NULL)
    {
      ...
      WRITE_LOG (NULL, 0, buf, _DBGOVERNOR_BUFFER_512,
		 "Connection already exists", data_cfg.log_mode);
      return -1;
    }
...
flush_user_priv (MODE_TYPE debug_mode)
{
  ...
  if (db_mysql_exec_query (QUERY_FLUSH_USER_PRIV, &mysql_do_command,
	...
	{
	  WRITE_LOG (NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048,
		     "Can't flush user privs", data_cfg.log_mode);
	}
      else
	{
	  WRITE_LOG (NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048,
  ...
  if (db_connect_common (&mysql_send_governor, host, user_name, user_password,
      ...
      else
	{
	  if (db_connect_common (&mysql_do_kill, host, user_name,
	    ...
	    {
	      WRITE_LOG (NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048,
			 "Kill connection error", data_cfg.log_mode);
	      return -1;
	    }
	}
...
check_mysql_version (MODE_TYPE debug_mode)
{
  ...
  if (mysql_send_governor != NULL)
      ...
      else
	{
	  (*_mysql_free_result) (res);
	  WRITE_LOG (NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048,
		     "Unknown mysql version", data_cfg.log_mode);
	  return 0;
	}
    }
  else
    {
      ...
      WRITE_LOG (NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048,
		 "Connection to db is absent", data_cfg.log_mode);
      return 0;
    }
}
...
log_user_queries (char *user_name, MODE_TYPE debug_mode)
{
  ...
  if (create_dir () && counts > 0)
      ...
      else
	{
	  (*_mysql_free_result) (res);
	  WRITE_LOG (NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048,
		     "Can't open file %s", data_cfg.log_mode, file_name);
	  return;
	}
    }
  (*_mysql_free_result) (res);
