...
db_connect_common(MYSQL ** internal_db, const char *host, const char *user_name, const char *user_password,
    ...
    if (*internal_db != NULL) {
        //Already connected
        WRITE_LOG(NULL, 0, buf, _DBGOVERNOR_BUFFER_512, "Connection already exists", data_cfg.log_mode);
        return -1;
    }
    ...
    *internal_db = (*_mysql_init) (NULL);
    if (*internal_db == NULL) {
        WRITE_LOG(NULL, 0, buf, _DBGOVERNOR_BUFFER_512, "Can't init mysql structure", data_cfg.log_mode);
        return -1;
    }
    ...
    if (_load_defaults) {
        ...
        while ((c = getopt_long(argc, argv, ":S:u:p:h:s:m:l:c:", long_options, &option_index)) != EOF) {
            switch (c) {
                case 'S':
    ...
    WRITE_LOG(NULL, 0, buf, _DBGOVERNOR_BUFFER_512, "Try to connect with options from dbgovernor config file", data_cfg.log_mode);
    ...
    if (!(*_mysql_real_connect) (*internal_db, host, user_name, user_password, db_name, 0, unix_socket_address, 0)) {
        //Previous attempt failed, try with data from my.cnf, .my.cnf
        WRITE_LOG(NULL, 0, buf, _DBGOVERNOR_BUFFER_512, "Try to connect with no password under root", data_cfg.log_mode);
        //Error again, stop to try
        WRITE_LOG(NULL, 0, buf, _DBGOVERNOR_BUFFER_512, db_getlasterror(*internal_db), data_cfg.log_mode);
        //Try to connect again
        if (user)
            strlcpy(work_user, user, USERNAMEMAXLEN);
        (*_mysql_options) (*internal_db, MYSQL_OPT_RECONNECT, &reconnect);
        if (!(*_mysql_real_connect) (*internal_db, host, user, password, db_name, 0, unix_socket_address, 0)) {
            //Error again, stop to try
            WRITE_LOG(NULL, 0, buf, _DBGOVERNOR_BUFFER_512, db_getlasterror(*internal_db), data_cfg.log_mode);

            WRITE_LOG(NULL, 0, buf, _DBGOVERNOR_BUFFER_512, "Try to connect with no password, no host, no user under root", data_cfg.log_mode);
            (*_mysql_options) (*internal_db, MYSQL_OPT_RECONNECT, &reconnect);
            if (!_load_defaults) {
                if ((*_mysql_options)(*internal_db, MYSQL_READ_DEFAULT_GROUP, "client")) {
                    if ((*_mysql_options)(*internal_db, MYSQL_READ_DEFAULT_GROUP, "mysqld")) {
            ...
            if (!(*_mysql_real_connect) (*internal_db, NULL, NULL, NULL, NULL, 0, unix_socket_address, 0)) {
                //Error again, stop to try
                WRITE_LOG(NULL, 0, buf, _DBGOVERNOR_BUFFER_512, db_getlasterror(*internal_db), data_cfg.log_mode);
                return -1;
            }
...
static int local_reconnect(MYSQL ** mysql_internal, MODE_TYPE debug_mode) {
    ...
    *mysql_internal = (*_mysql_init) (NULL);
    if (*mysql_internal == NULL) {
        WRITE_LOG(NULL, 0, buf, _DBGOVERNOR_BUFFER_512, "Can't init mysql structure(on reconnect)", data_cfg.log_mode);
        return -1;
    }
    ...
    if (!(*_mysql_real_connect) (*mysql_internal, global_host, unm, upwd, global_db_name, 0, unix_socket_address, 0)) {
        (*_mysql_options) (*mysql_internal, MYSQL_OPT_RECONNECT, &reconnect);
        if (!_load_defaults) {
        ...
        if (!(*_mysql_real_connect) (*mysql_internal, NULL, NULL, NULL, NULL, 0, unix_socket_address, 0)) {
            //Error again, stop to try
            WRITE_LOG(NULL, 0, buf, _DBGOVERNOR_BUFFER_512, db_getlasterror(*mysql_internal), data_cfg.log_mode);
            return -1;
        }
    }
    return 0;
    ...
    while ((*_mysql_query) (*mysql_internal, query)) {
        ...
        if (execution_counters == 0) {
            if (debug_mode != DEBUG_MODE) {
                WRITE_LOG(NULL, 0, buf, _DBGOVERNOR_BUFFER_2048, db_getlasterror(*mysql_internal), data_cfg.log_mode);
            } else {
                WRITE_LOG(NULL, 0, buf, _DBGOVERNOR_BUFFER_2048, "%s --- Request: %s",
...
void unfreaze_all(MODE_TYPE debug_mode) {
    ...
    snprintf(sql_buffer, _DBGOVERNOR_BUFFER_2048 - 1, QUERY_USER_CONN_LIMIT_UNFREEZE, (unsigned long) - 1);
...
void unfreaze_lve(MODE_TYPE debug_mode) {
    if (db_mysql_exec_query(QUERY_USER_CONN_LIMIT_UNFREEZE_LVE, &mysql_do_command, debug_mode))
        return;
    flush_user_priv(debug_mode);
}
...
void unfreaze_daily(MODE_TYPE debug_mode) {
    ...
    if (mysql_do_command == NULL)
        return;
    snprintf(buffer, _DBGOVERNOR_BUFFER_2048 - 1, QUERY_USER_CONN_LIMIT_UNFREEZE_DAILY, (unsigned long) - 1);
...
void update_user_limit_no_flush(char *user_name, unsigned int limit, MODE_TYPE debug_mode) {
    ...
    if (db_mysql_exec_query(sql_buffer, &mysql_do_command, debug_mode)) {
        if (debug_mode != DEBUG_MODE) {
            WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "Can't execute sql request. Restriction aborted", data_cfg.log_mode);
        } else {
            WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "%s --- Request: %s",
                    data_cfg.log_mode, "Can't execute sql request. Restriction aborted", sql_buffer);
        }
...
void flush_user_priv(MODE_TYPE debug_mode) {
    ...
    if (db_mysql_exec_query(QUERY_FLUSH_USER_PRIV, &mysql_do_command, debug_mode)) {
        if (debug_mode != DEBUG_MODE) {
            WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "Can't flush user privs", data_cfg.log_mode);
        } else {
            WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "%s --- Request: %s",
...
kill_query(char *user_name, MODE_TYPE debug_mode) {
    ...
    if (db_mysql_exec_query(sql_buffer, &mysql_do_command, debug_mode)) {
        if (debug_mode != DEBUG_MODE) {
            WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "Can't execute sql request. Kill query", data_cfg.log_mode);
        } else {
            WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "%s --- Request: %s",
...
void kill_query_by_id(long id, MODE_TYPE debug_mode, MYSQL ** mysql_internal) {
    ...
    if (db_mysql_exec_query(sql_buffer, mysql_internal, debug_mode)) {
        ...
        if (debug_mode != DEBUG_MODE) {
            ...
            WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "%s --- Request: %s", data_cfg.log_mode,
...
void governor_enable(MODE_TYPE debug_mode) {
    ...
    if (is_plugin_version) {
        if (db_mysql_exec_query(QUERY_GOVERNOR_MODE_ENABLE_PLG, &mysql_send_governor, debug_mode)) {
            WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "Can't execute sql request. ENABLE_GOVERNOR_PLG", data_cfg.log_mode);
        }
    } else {
        if (db_mysql_exec_query(QUERY_GOVERNOR_MODE_ENABLE, &mysql_send_governor, debug_mode)) {
            WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "Can't execute sql request. ENABLE_GOVERNOR", data_cfg.log_mode);
        }
    }
...
void governor_enable_reconn(MODE_TYPE debug_mode) {
    ...
    if (is_plugin_version) {
        if (db_mysql_exec_query(QUERY_GOVERNOR_MODE_ENABLE_RECON_PLG, &mysql_send_governor, debug_mode)) {
            WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "Can't execute sql request. ENABLE_GOVERNOR", data_cfg.log_mode);
        }
    } else {
        if (db_mysql_exec_query(QUERY_GOVERNOR_MODE_ENABLE_RECON, &mysql_send_governor, debug_mode)) {
            WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "Can't execute sql request. ENABLE_GOVERNOR", data_cfg.log_mode);
        }
    }
    if (mysql_send_governor) {
...
void governor_enable_lve(MODE_TYPE debug_mode) {
    ...
    if (is_plugin_version) {
        if (db_mysql_exec_query(QUERY_GOVERNOR_MODE_ENABLE_LVE_PLG, &mysql_send_governor, debug_mode)) {
            WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "Can't execute sql request. ENABLE_GOVERNOR", data_cfg.log_mode);
        }
    } else {
        if (db_mysql_exec_query(QUERY_GOVERNOR_MODE_ENABLE_LVE, &mysql_send_governor, debug_mode)) {
            WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "Can't execute sql request. ENABLE_GOVERNOR", data_cfg.log_mode);
        }
    }
    if (mysql_send_governor) {
...
void governor_enable_reconn_lve(MODE_TYPE debug_mode) {
    ...
    if (is_plugin_version) {
        if (db_mysql_exec_query(QUERY_GOVERNOR_MODE_ENABLE_RECON_LVE_PLG, &mysql_send_governor, debug_mode)) {
            WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "Can't execute sql request. ENABLE_GOVERNOR", data_cfg.log_mode);
        }
    } else {
        if (db_mysql_exec_query(QUERY_GOVERNOR_MODE_ENABLE_RECON_LVE, &mysql_send_governor, debug_mode)) {
            WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "Can't execute sql request. ENABLE_GOVERNOR", data_cfg.log_mode);
        }
    }
    if (mysql_send_governor) {
...
void kill_connection(char *user_name, MODE_TYPE debug_mode) {
    ...
    if (db_mysql_exec_query(sql_buffer, &mysql_do_command, debug_mode)) {
        if (debug_mode != DEBUG_MODE) {
            WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "Can't execute sql request. Kill connection", data_cfg.log_mode);
        } else {
            WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "%s --- Request: %s", data_cfg.log_mode,
                    "Can't execute sql request. Kill connection", sql_buffer);
        }
...
int db_connect(const char *host, const char *user_name, const char *user_password, const char *db_name, int argc, char *argv[], MODE_TYPE debug_mode) {
    ...
    WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "Open govern connection operation", data_cfg.log_mode);

    if (db_connect_common(&mysql_send_governor, host, user_name, user_password, db_name, debug_mode, argc, argv, 1) < 0) {
        WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "Send governor connection error", data_cfg.log_mode);
        return -1;
    } else {
        WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "Open write connection operation", data_cfg.log_mode);
        if (db_connect_common(&mysql_do_command, host, user_name, user_password, db_name, debug_mode, argc, argv, 0) < 0) {
            WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "Write connection error", data_cfg.log_mode);
            return -1;
        } else {
            if (db_connect_common(&mysql_do_kill, host, user_name, user_password, db_name, debug_mode, argc, argv, 0) < 0) {
                WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "Kill connection error", data_cfg.log_mode);
                return -1;
            }
        }
    }
...
int check_mysql_version(MODE_TYPE debug_mode) {
    ...
    if (mysql_send_governor != NULL) {
        if (db_mysql_exec_query(QUERY_GET_SERVER_INFO, &mysql_send_governor, debug_mode)) {
            WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "Get mysql vesrion request failed", data_cfg.log_mode);
            return 0;
        }
        res = (*_mysql_store_result) (mysql_send_governor);
        ...
        if (row) {
            ...
            if (strstr(buffer, "-cll-lve")) {
                ...
                WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, outbuffer, data_cfg.log_mode);
                return 1;
            } else {
                if (db_mysql_exec_query(QUERY_GOVERNOR_CHECK_PLUGIN, &mysql_send_governor, debug_mode)) {
                    WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "Get mysql vesrion request failed", data_cfg.log_mode);
                    return 0;
                }
                res = (*_mysql_store_result) (mysql_send_governor);
                ...
                if (row) {
                    ...
                    snprintf(outbuffer, _DBGOVERNOR_BUFFER_2048 - 1,
                            "Update your MySQL to CLL version from repo.cloudlinux.com or install plugin. Current is %s", buffer);
                    WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, outbuffer, data_cfg.log_mode);
                    (*_mysql_free_result) (res);
                    return 1;
                }
            ...
            (*_mysql_free_result) (res);
            WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "Unknown mysql version", data_cfg.log_mode);
            return 0;
        }
    } else {
        WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "Connection to db is absent", data_cfg.log_mode);
        return 0;
    }
}
...
void lve_connection(char *user_name, MODE_TYPE debug_mode) {
    ...
    if (db_mysql_exec_query(sql_buffer, &mysql_do_command, debug_mode)) {
        if (debug_mode != DEBUG_MODE) {
            WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "Can't execute sql request. LVE connection", data_cfg.log_mode);
        } else {
            WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "%s --- Request: %s", data_cfg.log_mode,
...
int activate_plugin(MODE_TYPE debug_mode) {
    if (is_plugin_version) {
        ...
        if (mysql_send_governor != NULL) {
            if (db_mysql_exec_query(QUERY_GET_PLUGIN_INFO, &mysql_send_governor, debug_mode)) {
                WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "Get mysql plugin request failed", data_cfg.log_mode);
                return 0;
            }
            res = (*_mysql_store_result) (mysql_send_governor);
            while ((row = (*_mysql_fetch_row) (res))) {
            ...
            (*_mysql_free_result) (res);
            if (!is_founf_plg) {
                if (db_mysql_exec_query(QUERY_SET_PLUGIN_INFO, &mysql_send_governor, debug_mode)) {
                    if (!strstr((char *) (*_mysql_error) (mysql_send_governor), "Function 'governor' already exists")) {
                    }
                    {
                        WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "Set mysql plugin request failed", data_cfg.log_mode);
                        return 0;
                    }
                }
                res = (*_mysql_store_result) (mysql_do_command);
