...
static char *mode_type_enum_to_str[] = {
    "TEST_MODE",
    "PRODUCTION_MODE",
    "DEBUG_MODE",
    "ERROR_MODE",
...
int write_log(const char *error_file, int error_line, const char *error_string, MODE_TYPE mode) {
    if (log == NULL)
        return -1;
    ...
    if (mode == DEBUG_MODE)
        rc = fprintf(log, "[%s] %s:%d %s\n", current_date, error_file, error_line, error_string);
    else
...
int write_restrict_log_second_line(const char *error_string, int need_end_line) {
    int rc = fprintf(restrict_log, "%s", error_string);
