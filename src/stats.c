...
User_stats *init_user_stats(const char *id, Account * ac) {
    ...
    us->id = id;
    us->account = ac;
    us->stats = fifo_stats_init(COMPARE_MAX_3(data_cfg.interval_long, data_cfg.interval_mid, data_cfg.interval_short));
    return us;
}
...
void call_diff_stats(Stats * st, User_stats * us) {
}

void save_stats(Stats * dst, Stats * st) {
    dst->cpu = st->cpu;
    dst->read = st->read;
    dst->write = st->write;
}
