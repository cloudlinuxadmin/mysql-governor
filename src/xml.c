 ...
 * node finding or NULL for docroot
 */
void *FindElementWithNameAndAttr(xml_data *data, void *node, const char *nodeName, const char *attrName, const char *attrValue) {
    xmlNodePtr cur = node ? node : (xmlNodePtr) data->root;
    cur = cur->xmlChildrenNode;
    while (cur != NULL) {
        if ((!xmlStrcmp(cur->name, (const xmlChar *) nodeName))) {
...
void *getNextChild(void *node, const char *childName, void *prev_node) {
    ...
    xmlNodePtr cur = prev_node ? ((xmlNodePtr) prev_node)->next : nodef->xmlChildrenNode;
    while (cur != NULL) {
...
const char *getElemAttr(void *node, const char *attrName) {
    ...
    while (attribute) {
        if ((!xmlStrcmp(attribute->name, (const xmlChar *) attrName))) {
            xmlChar* value = xmlNodeListGetString(((xmlNodePtr) node)->doc, attribute->children, 1);
...
const char *getElemValue(void *node) {
    xmlChar* value = xmlNodeListGetString(((xmlNodePtr) node)->doc, ((xmlNodePtr) node)->xmlChildrenNode, 1);
    return (const char *) value;
}
 ...
 */
void *setNode(xml_data *data, void *node, const char *nodeName, const char *value) {
    ...
    if (fNode) {
        ...
        addressNode = xmlNewChild(parent, NULL, (const xmlChar *) nodeName, (const xmlChar *) value);
    }
...
void *setAttr(void *node, const char *attrName, const char *value) {
    ...
    if (node) {
        addressNode = xmlSetProp((xmlNodePtr) node, (const xmlChar *) attrName, (const xmlChar *) value);
    }
    return (void *) addressNode;
}
