...
void account_restrict(Account * ac, stats_limit_cfg * limit) {
    ...
    if (data_cfg.exec_script) {
        ...
        snprintf(limValue, _DBGOVERNOR_BUFFER_128, "%ld", getLimitValue(ac, limit));
        ...
        if (trigger_pid < 0) {
            ...
            if (!trigger_pid) {
                execl(data_cfg.exec_script, data_cfg.exec_script, ac->id, period_name, varName, varValue, limValue,
                ...
                if (data_cfg.use_lve) {
                    if (add_user_to_list(cmd->username, data_cfg.all_lve) < 0) {
                        ...
                        if (data_cfg.max_user_connections) {
                            update_user_limit_no_flush(cmd->username, (unsigned int) data_cfg.max_user_connections, data_cfg.log_mode);
                            is_any_flush = 1;
                        }
                    }
                } /*else
