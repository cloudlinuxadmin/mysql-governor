...
int inputAvailable(FILE * fp) {
    ...
    while (select(fileno(fp) + 1, &fds, NULL, NULL, &tv) < 0) {
        if (errno != EINTR)
            break;
    }
...
size_t fread_wrapper(void *__restrict __ptr, size_t __size, size_t __n, FILE * __restrict __s) {
    size_t res = 0;
