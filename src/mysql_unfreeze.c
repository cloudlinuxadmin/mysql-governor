...
int main(int argc, char *argv[]) {
    ...
    if (cmd == 3) {
        ...
        printf("     empty command        :unfreeze users according to configuration file options\n");
        printf("     force-old            :unfreeze users by DBDISABLE method (for old restriction removing)\n");
        printf("     show-bad-users-list  :show bad users list (if lve using)\n");
        return 0;
    }
    ...
    if (!data_cfg.is_gpl && data_cfg.use_lve && (cmd != 1)) {
        if (cmd == 0) {
            ...
            if (init_bad_users_list_utility() >= 0) {
                WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "Unfreeze completed", data_cfg.log_mode);
                remove_bad_users_list_utility();
            } else {
                WRITE_LOG(NULL, 0, buffer, _DBGOVERNOR_BUFFER_2048, "Can't init BAD users list", data_cfg.log_mode);
            }
            db_close();
        } else {
