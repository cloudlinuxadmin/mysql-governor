...
int init_bad_users_list_utility() {
    ...
    bad_list = (shm_structure *) cl_mmap(0, sizeof (shm_structure), (PROT_READ | PROT_WRITE), MAP_SHARED, shm_fd, 0);
    if (bad_list == MAP_FAILED) {
        close(shm_fd);
        return -1;
    }
...
int init_bad_users_list() {
    ...
    int first = 0;
    if ((shm_fd = shm_open(SHARED_MEMORY_NAME, (O_CREAT | O_EXCL | O_RDWR), 0755)) > 0) {
        first = 1;
    } else if ((shm_fd = shm_open(SHARED_MEMORY_NAME, (O_CREAT | O_RDWR), 0755)) < 0) {
        umask(old_umask);
        return -1;
    }
        ...
        if (!strncmp(bad_list->items[index].username, username, USERNAMEMAXLEN)) {
            if (sem_wait(sem) == 0) {
                if (index == (bad_list->numbers - 1)) {
                    ...
                    memmove(bad_list->items + index, bad_list->items + (index + 1),
...
int32_t is_user_in_bad_list_cleint(char *username) {
    ...
    bad_list_clents = (shm_structure *) cl_mmap(0, sizeof (shm_structure), PROT_READ, MAP_SHARED, shm_fd_clents, 0);
    if (bad_list_clents == MAP_FAILED) {
        close(shm_fd_clents);
        return 0;
    }
            ...
            if (sem_trywait(sem_client) == 0) {
                if (bad_list_clents) {
                    ...
                    for (index = 0; index < bad_list_clents->numbers; index++) {
                        if (!strncmp(bad_list_clents->items[index].username, username, USERNAMEMAXLEN)) {
...
int init_bad_users_list_client() {
    ...
    if ((shm_fd_clents_global = shm_open(SHARED_MEMORY_NAME, (O_CREAT | O_EXCL | O_RDWR), 0600)) > 0) {
        first = 1;
    } else if ((shm_fd_clents_global = shm_open(SHARED_MEMORY_NAME, (O_CREAT | O_RDWR), 0600)) < 0) {
        ...
        while (trys) {
            if (sem_trywait(sem_client) == 0) {
                if (bad_list_clents_global && (bad_list_clents_global != MAP_FAILED)) {
                    long index = 0;
                    for (index = 0; index < bad_list_clents_global->numbers; index++)
                        printf(" %-16s %ld\n", bad_list_clents_global->items[index].username, index);
                }
                trys = 0;
            } else {
                if (errno == EAGAIN) {
