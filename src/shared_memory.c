...
int
init_bad_users_list_if_not_exitst ()
{
  if (!bad_list || (bad_list == MAP_FAILED))
    {
...
is_user_in_list (char *username)
{
  ...
  for (index = 0; index < bad_list->numbers; index++)
    {
      if (!strncmp
	  (bad_list->items[index].username, username, USERNAMEMAXLEN))
	return 1;
    }
  return 0;
}
...
int
add_user_to_list (char *username, int is_all)
{
  if (!bad_list || (bad_list == MAP_FAILED))
    return -1;
  int uid = BAD_LVE;
...
int
delete_user_from_list (char *username)
{
  if (!bad_list || (bad_list == MAP_FAILED))
    return -1;
  long index;
...
int32_t
is_user_in_bad_list_cleint (char *username)
{
  int shm_fd_clents = 0;
  int32_t fnd = 0;
	  ...
	  if (sem_trywait (sem_client) == 0)
	      ...
	      if (bad_list_clents)
		{
		  ...
		  for (index = 0; index < bad_list_clents->numbers; index++)
		    {
		      if (!strncmp
			  (bad_list_clents->items[index].username, username,
			   USERNAMEMAXLEN))
			{
...
init_bad_users_list_client ()
{
  ...
  int first = 0;
  if ((shm_fd_clents_global =
       shm_open (SHARED_MEMORY_NAME, (O_CREAT | O_EXCL | O_RDWR), 0600)) > 0)
    {
      first = 1;
    }
  else
    if ((shm_fd_clents_global =
	 shm_open (SHARED_MEMORY_NAME, (O_CREAT | O_RDWR), 0600)) < 0)
    {
	  ...
	  if (sem_trywait (sem_client) == 0)
	    {
		  ...
		  && (bad_list_clents_global != MAP_FAILED))
		{
		  long index = 0;
		  for (index = 0; index < bad_list_clents_global->numbers;
		       index++)
		    {
		      if (!strncmp
