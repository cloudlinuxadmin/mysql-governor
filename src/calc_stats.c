...
void add_empty_stats_for_uid(username_t username) {
    ...
    if (g_hash_table_lookup(users, username) == NULL)
...
void calc_acc_stats(gpointer key, Account * ac, gpointer data) {
    ...
    while (i < ac->users->len) {
        ...
        if (ac->need_dbg) {
            ...
            WRITE_LOG(NULL, 1, output_buffer, _DBGOVERNOR_BUFFER_2048,
                    " step 2-%d: proceed user stats %s, c %f, r %llu, w %llu",
                    internal_info->log_mode, i, us->id ? us->id : "Unk",
                    fifo_stats_get(us->stats, 0)->cpu,
    ...
    if (ac->need_dbg) {
        ...
        WRITE_LOG(NULL, 1, output_buffer, _DBGOVERNOR_BUFFER_2048,
                ...
                internal_info->log_mode, ac->current.cpu, ac->current.read, ac->current.write);
    }
}
...
void IncNumberOfRestricts(const char *username, int cause) {
    statistics_restrict_info *DbGovNumberOfRestricts_ = NULL;
    if (DbGovNumberOfRestricts == NULL)
        DbGovNumberOfRestricts = g_hash_table_new_full(g_str_hash, g_str_equal, NULL, free);

    DbGovNumberOfRestricts_ = (statistics_restrict_info *) g_hash_table_lookup(DbGovNumberOfRestricts, username);
...
int get_cause_of_restrict(GOVERNORS_FIELD_NAME _cur) {
    switch (_cur) {
        case CPU_PARAM:
...
int check_restrict(Account * ac) {
    ...
    if (_cur != NORESTRICT_PARAM2) {
        ...
        if (data_cfg.restrict_log) {
            ...
            WRITE_LOG(&ac->current, 1, tmp_buf, _DBGOVERNOR_BUFFER_8192, tmp_buf, data_cfg.log_mode);
        }
        ...
        if (_short != NORESTRICT_PARAM2) {
            ...
            if (data_cfg.restrict_log) {
                char tmp_buf[_DBGOVERNOR_BUFFER_8192];
                prepareRestrictDescription(tmp_buf, ac, sl);
                WRITE_LOG(&ac->short_average, 1, tmp_buf, _DBGOVERNOR_BUFFER_8192, tmp_buf, data_cfg.log_mode);
            }

            return 1;
        } else {
            GOVERNORS_FIELD_NAME _mid = is_stat_overlimit_mid(&ac->mid_average, sl);
            if (_mid != NORESTRICT_PARAM2) {
                ...
                if (data_cfg.restrict_log) {
                    char tmp_buf[_DBGOVERNOR_BUFFER_8192];
                    prepareRestrictDescription(tmp_buf, ac, sl);
                    WRITE_LOG(&ac->mid_average, 1, tmp_buf, _DBGOVERNOR_BUFFER_8192, tmp_buf, data_cfg.log_mode);
                }

                return 1;
            } else {
                GOVERNORS_FIELD_NAME _long = is_stat_overlimit_long(&ac->long_average, sl);
                if (_long != NORESTRICT_PARAM2) {
                    ...
                    if (data_cfg.restrict_log) {
                        char tmp_buf[_DBGOVERNOR_BUFFER_8192];
                        prepareRestrictDescription(tmp_buf, ac, sl);
                        WRITE_LOG(&ac->long_average, 1, tmp_buf, _DBGOVERNOR_BUFFER_8192, tmp_buf, data_cfg.log_mode);
                    }
    ...
    if (sl->mode != IGNORE_MODE) {
        ...
        if (ac->start_count > 0) {
            ...
            if (check_restrict_value) {
                ...
                WRITE_LOG(NULL, 1, tmp_buf, _DBGOVERNOR_BUFFER_8192, tmp_buf, data_cfg.log_mode);
            }
        }
    ...
    if (internal_info->dbg) {
        ...
        if (!strncmp(internal_info->dbg, (char *) key, len)) {
            ...
            WRITE_LOG(NULL, 1, output_buffer, _DBGOVERNOR_BUFFER_2048,
                    ...
                    internal_info->log_mode, item->s.cpu, item->s.read, item->s.write, (*in_tm - item->tm));
        }
    }
...
static int dbgov_was_user_activity(dbgov_statitrics * dbgovst) {
    if ((dbgovst->sum_cpu > 0.0 || dbgovst->sum_write > 0.0 || dbgovst->sum_read > 0.0 || dbgovst->cause)
...
void dbstat_print_table(gpointer key, dbgov_statitrics * dbgov_statitrics__, void *data) {
    ...
    int number_of_iterations = dbgov_statitrics__->number_of_iterations, mb_s = 1000000;
...
void dbstat_add_to_table(gpointer key, Account * ac, void *data) {
    stats_limit_cfg cfg_buf;
    dbgov_statitrics *dbgov_statitrics__ = NULL;
    dbgov_statitrics__ = (dbgov_statitrics *) g_hash_table_lookup(DbGovStatitrics, ac->id);
    stats_limit_cfg *sl = config_get_account_limit(ac->id, &cfg_buf);
    ...
    if (DbGovNumberOfRestricts)
        number_of_restricts = (statistics_restrict_info *) g_hash_table_lookup(DbGovNumberOfRestricts, ac->id);
    ...
    if (dbgov_statitrics__ == NULL) {
        ...
        dbgov_statitrics__->number_of_restricts = number_of_restricts ? number_of_restricts->number_of_restricts : 0;
        dbgov_statitrics__->cause = number_of_restricts ? number_of_restricts->cause_of_restricts : 0;
        ...
        dbgov_statitrics__->max_cpu = ac->current.cpu > dbgov_statitrics__->max_cpu ? ac->current.cpu : dbgov_statitrics__->max_cpu;
        dbgov_statitrics__->max_write = ac->current.write > dbgov_statitrics__->max_write ? ac->current. write : dbgov_statitrics__->max_write;
        dbgov_statitrics__->max_read = ac->current.read > dbgov_statitrics__->max_read ? ac->current.read : dbgov_statitrics__->max_read;
...
double AddDbGovStatitrics(double old_tm) {
    ...
    if (new_tm - old_tm >= 1.0) {
        if (DbGovStatitrics == NULL)
            DbGovStatitrics = g_hash_table_new_full(g_str_hash, g_str_equal, NULL, free);
...
void clac_stats_difference_inner(long long cpu, long long read, long long write, tid_table * old, Stats * st) {
    ...
    if (new_tm > old_tm) {
        ...
        if (cpu >= old->cpu) {
            ...
            //      st->cpu = 0.0;
            //else
            st->cpu = ((double) (cpu - old->cpu) / tm) / ((double) sysconf(_SC_CLK_TCK));
        } else {
            st->cpu = 0.0;
        }
...
void clac_stats_difference_inner_from_counter(long long cpu, long long read, long long write, double tm_in, Stats * st, double new_tm) {
    ...
    if (tm > 0.0) {
        ...
        if (cpu > 0) {
            ...
            //      st->cpu = 0.0;
            //else
            st->cpu = ((double) (cpu) / (tm)) / ((double) sysconf(_SC_CLK_TCK));
        } else {
            st->cpu = 0.0;
        }
...
void clac_stats_difference_add_to_counters(client_data * new, tid_table * old) {
    clac_stats_difference_inner_add_to_counters(new->cpu, new->read, new->write, old);
}
    ...
    if (new_tm > old_tm) {
        ...
        if ((new_tm - old_tm) > 1.0)
            increment_counters(old->username,
                (long long) floor((double) tmp_cpu / (new_tm - old_tm)),
                (long long) floor((double) tmp_read / (new_tm - old_tm)),
                (long long) floor((double) tmp_write / (new_tm - old_tm)),
                old_tm);
        else
            increment_counters(old->username, tmp_cpu, tmp_read, tmp_write, old_tm);
    } else {
...
void print_to_restrict_log_account_info(gpointer * key, Account * ac, void *data) {
    ...
    WRITE_LOG(NULL, 1, buffer, _DBGOVERNOR_BUFFER_2048,
            ...
            ac->short_average.cpu, ac->short_average.read, ac->short_average.write,
            ac->mid_average.cpu, ac->mid_average.read, ac->mid_average.write,
            ac->long_average.cpu, ac->long_average.read, ac->long_average.write);
}
...
void print_to_restrict_log_stats(void *data) {
    ...
    time_t tm = time(NULL);
    WRITE_LOG(NULL, 1, buffer, _DBGOVERNOR_BUFFER_2048, "Begin screen %ld", data_cfg.log_mode, tm);
    g_hash_table_foreach(accounts, (GHFunc) print_to_restrict_log_account_info, NULL);
    WRITE_LOG(NULL, 1, buffer, _DBGOVERNOR_BUFFER_2048, "End screen %ld", data_cfg.log_mode, tm);
}
...
void dbctl_restrict_set(gpointer key, Account * ac, void *data) {
    ...
    if ((strncmp(ac->id, command->options.username, USERNAMEMAXLEN) == 0) || (comapre_users_name(command->options.username, ac) == 0)) {
        stats_limit_cfg cfg_buf;
        stats_limit_cfg *sl = config_get_account_limit(ac->id, &cfg_buf);
        char restrict_bufer[_DBGOVERNOR_BUFFER_4096];
        ...
        if (data_cfg.statistic_mode)
            if (level >= 0)
                IncNumberOfRestricts(ac->id, RESTRICT_BY_CPU | RESTRICT_BY_READ | RESTRICT_BY_WRITE);
