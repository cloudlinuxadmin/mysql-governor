...
void dbstat_add_to_table( gpointer key, Account *ac, void *data )
{
  ...
  if (DbGovNumberOfRestricts) number_of_restricts = (statistics_restrict_info *)g_hash_table_lookup( DbGovNumberOfRestricts, ac->id );
  ...
  {   
    ...
    dbgov_statitrics__->max_cpu = ac->current.cpu > dbgov_statitrics__->max_cpu ? ac->current.cpu : 
                                                                                  dbgov_statitrics__->max_cpu ;
    dbgov_statitrics__->max_write = ac->current.write > dbgov_statitrics__->max_write ? ac->current.write : 
                                                                                        dbgov_statitrics__->max_write ;
    dbgov_statitrics__->max_read = ac->current.read > dbgov_statitrics__->max_read ? ac->current.read : 
...
double AddDbGovStatitrics( double old_tm )
{
  ...
  if( new_tm - old_tm >= 60.0 )
  {
    ...
    {
      if(DbGovStatitrics) g_hash_table_remove_all( DbGovStatitrics );
      if(DbGovNumberOfRestricts) g_hash_table_remove_all( DbGovNumberOfRestricts );
      old_tm = new_tm;
    }
  }
...
void clac_stats_difference_inner_from_counter(long long cpu, long long read,
		long long write, double tm_in, Stats *st, double new_tm) {
...
void clac_stats_difference_inner_add_to_counters(long long cpu, long long read,
	...
	if (new_tm > old_tm) {
		...
		if(cpu>=old->cpu) tmp_cpu = cpu - old->cpu;
		if(read>=old->read) tmp_read = read - old->read;
		if(write>=old->write) tmp_write = write - old->write;
		if((new_tm - old_tm)>1.0)
		    increment_counters(old->username, (long long)floor((double)tmp_cpu/(new_tm-old_tm)) , (long long)floor((double)tmp_read/(new_tm-old_tm)), (long long)floor((double)tmp_write/(new_tm-old_tm)), old_tm);
		else
...
void print_to_restrict_log_account_info(gpointer * key, Account * ac,
		void *data) {
