...
GPtrArray *addMemoryUser(FILE * in, GPtrArray * tags) {
    ...
    for (ac = g_list_first(list); ac != NULL; ac = g_list_next(ac)) {
        ...
        found_tag_list->attr = g_hash_table_new_full(g_str_hash, g_str_equal, (GDestroyNotify) found_tag_key_destroyed,
...
void print_list(FILE * in, int flag, int non_priv) {
    DbCtlLimitAttr cpu_def, read_def, write_def;
    char val = flag == 1 ? val = 'K' : (flag == 2 ? ' ' : 'M');
...
const char *read_restrict_reriod(Account * ac) {
...
const char *read_restrict_reason(Account * ac) {
    if (ac->info.field_restrict == NO_PERIOD)
        return "";
...
int get_time_to_end(Account * ac) {
    return (((ac->start_count + ac->timeout) - time(NULL)) < 0) ? 0 : ((ac->start_count + ac->timeout) - time(NULL));
}
