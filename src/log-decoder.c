...
void insertSystemInfo(char *buffer) {
    ...
    getvmstat(vmstat);
    snprintf(innerBuffer, _DBGOVERNOR_BUFFER_8192, "%s loadavg(%s) vmstat(%s)", buffer, loadavg, vmstat);
    strlcpy(buffer, innerBuffer, _DBGOVERNOR_BUFFER_8192);
}

void prepareRestrictDescription(char *buffer, Account * ac, stats_limit_cfg * limit) {
    char ch[32];
    ...
    if (ac->info.field_restrict == NO_PERIOD) {
        ...
        sprintf(buffer, "%s LIMIT_ENFORCED period %s, field %s value %llu/limit %ld restrict level %d",
                ac->id, ch, varName, (unsigned long long) getRestrictValue(ac), getLimitValue(ac, limit), ac->restricted + 1);
        if (cfg->restrict_format >= 2)
            insertSystemInfo(buffer);
...
void prepareRestrictDescriptionLimit(char *buffer, Account * ac, stats_limit_cfg * limit) {
    char ch[32];
    char varName[_DBGOVERNOR_BUFFER_128];
    strcpy(buffer, "");
    if (ac->info.field_restrict == NO_PERIOD) {
        ...
        getPeriodName(ch, ac);
        getParamName(varName, ac);
        sprintf(buffer, "%s LIMIT_ENFORCED period %s, field %s value %llu/limit %ld",
                ac->id, ch, varName, (unsigned long long) getRestrictValue(ac), getLimitValue(ac, limit));
