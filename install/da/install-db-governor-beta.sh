...
if [ "$SQL_VERSION" == "auto" ]; then
	if [ "${MYSQL_OPT}" == "5.1" ]; then
		wget -O /etc/yum.repos.d/cl-mysql.repo  http://repo.cloudlinux.com/other/$CL_VER/mysqls/cl-mysql-5.1-common.repo
	        sed '/userstat/d' -i /etc/my.cnf
		sed '/userstat_running/d' -i /etc/my.cnf
	else
		wget -O /etc/yum.repos.d/cl-mysql.repo  http://repo.cloudlinux.com/other/$CL_VER/mysqls/cl-mysql-${MYSQL_OPT}-common.repo
	fi
fi
if [ "$SQL_VERSION" == "mysql50" ]; then
	wget -O /etc/yum.repos.d/cl-mysql.repo  http://repo.cloudlinux.com/other/$CL_VER/mysqls/cl-mysql-5.0-common.repo
fi
...
if [ "$SQL_VERSION" == "mysql55" ]; then
	wget -O /etc/yum.repos.d/cl-mysql.repo  http://repo.cloudlinux.com/other/$CL_VER/mysqls/cl-mysql-5.5-common.repo
fi
if [ "$SQL_VERSION" == "mysql56" ]; then
	wget -O /etc/yum.repos.d/cl-mysql.repo  http://repo.cloudlinux.com/other/$CL_VER/mysqls/cl-mysql-5.6-common.repo
fi
if [ "$SQL_VERSION" == "mariadb55" ]; then
	wget -O /etc/yum.repos.d/cl-mysql.repo  http://repo.cloudlinux.com/other/$CL_VER/mysqls/cl-mariadb-5.5-common.repo
fi
