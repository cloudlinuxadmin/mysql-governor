...
%setup -n governor-mysql2-%{version}
...
cd -
#mkdir -p %{buildroot}/var/lve/dbgovernor/
#mkdir -p %{buildroot}/var/lve/dbgovernor-store/
...
install -D -m 600 db-governor.xml %{buildroot}%{governor_path}/mysql-governor.xml
...
%endif
if [ ! -e %{_sysconfdir}/container/mysql-governor.xml ]; then
    cp %{governor_path}/mysql-governor.xml %{_sysconfdir}/container/mysql-governor.xml
    chmod 600 %{_sysconfdir}/container/mysql-governor.xml
fi
if [ ! -e /var/lve/dbgovernor/ ]; then
    mkdir -p /var/lve/dbgovernor/
fi
if [ ! -e /var/lve/dbgovernor-store/ ]; then
...
* Wed Nov 01 2017 Alexey Berezhok <alexey.berezhok@cloudlinux.com> - 1.2-27
- MYSQLG-221: Add installation of mysqlclient18 for MariaDB 10.2

* Mon Oct 23 2017 Alexey Berezhok <alexey.berezhok@cloudlinux.com> - 2.0-1.1
- Rebuild governor as MySQL plugin

* Fri Oct 20 2017 Daria Kavchuk <dkavchuk@cloudlinux.com> - 1.1-26
- MYSQLG-218: added MariaDB 10.2 support

* Tue Oct 17 2017 Alexey Berezhok <alexey.berezhok@cloudlinux.com> - 1.2-26
...
* Thu Sep 07 2017 Eugene Vodilov <evodilov@cloudlinux.com> - 1.2-25
